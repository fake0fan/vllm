...
class NixlECConnector(ECConnectorBase):
    ...
    def save_caches(
        ...
        if mm_hash in encoder_cache: 
            ...
            logger.debug(f"hero: base_addr: {base_addr}")

            if self.engine_id not in self.connector_worker._ENCODER_MM_BASE_ADDRS:
                self.connector_worker._ENCODER_MM_BASE_ADDRS[self.engine_id] = {}
                logger.debug(f"hero: self.connector_worker._ENCODER_MM_BASE_ADDRS: {self.connector_worker._ENCODER_MM_BASE_ADDRS}")

            self.connector_worker._ENCODER_MM_BASE_ADDRS[self.engine_id][mm_hash] = base_addr
            logger.debug(f"hero: self.connector_worker._ENCODER_MM_BASE_ADDRS: {self.connector_worker._ENCODER_MM_BASE_ADDRS}")
...
class NixlECConnectorScheduler:
    ...
    def update_mm_hash_addrs_from_output(self, ec_connector_output: ECConnectorOutput) -> None:
        logger.debug(f"update_mm_hash_addrs_from_output ec_connector_output.mm_hash_addrs: {ec_connector_output.mm_hash_addrs}")
        mm_hash_addrs = ec_connector_output.mm_hash_addrs
        logger.debug(f"hero: mm_hash_addrs: {mm_hash_addrs}")
        self._ENCODER_MM_BASE_ADDRS.update(mm_hash_addrs)
...
class NixlECConnectorWorker:
    ...
    def add_remote_agent(
        ...
        logger.debug(f"hero: add_remote_agent end; remote_agent_name: {remote_agent_name}")
    ...
    def _read_mm_segments(self, mm_hash: str, meta: ECReqMeta):
        ...
        logger.debug(f"hero: self._remote_agents: {self._remote_agents}")
        remote_engine_id = meta.remote_engine_id
