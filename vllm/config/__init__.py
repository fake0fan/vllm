...
class ECTransferConfig:
    ...
    ec_connector: Optional[str] = None
    ...
    ec_buffer_device: Optional[str] = "cuda"
    ...
    ec_role: Optional[ECRole] = None
    """Whether this vLLM instance produces, consumes KV cache, or both. Choices
    are 'ec_producer', 'ec_consumer'."""

    ec_rank: Optional[int] = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.ec_role is not None and self.ec_role not in get_args(ECRole):
            raise ValueError(f"Unsupported ec_role: {self.ec_role}. "
    ...
    def is_ec_transfer_instance(self) -> bool:
        return self.ec_connector is not None and \
            self.ec_role in get_args(ECRole)
